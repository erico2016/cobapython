...
class BrickSetSpider(scrapy.Spider):
    name = "http://web.beritaislamterbaru.org/"
    start_urls = ['http://web.beritaislamterbaru.org/']
    
    def parse(self, response):
        SET_SELECTOR = 'article.post.hentry'    
        for brickset in response.css(SET_SELECTOR):
            NAME_SELECTOR = 'h2.post-title.entry-title a ::text'
            DESKRIPSI_SELECTOR = 'div.post-body.entry-content::text' 
